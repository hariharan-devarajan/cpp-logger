...
# MACRO cpp_logger_add_c|cxx_flags
#
...
macro(cpp_logger_add_cxx_flags MY_FLAGS)
  foreach(flag ${ARGN})
...
macro(cpp_logger_add_c_flags MY_FLAGS)
  foreach(flag ${ARGN})
    string(FIND "${${MY_FLAGS}}" "${flag}" flag_already_set)
    if(flag_already_set EQUAL -1)
      string(REPLACE "-" "_" _CLEAN_FLAG "${flag}")
...
cpp_logger_add_cxx_flags(CMAKE_CXX_FLAGS
  -fPIC -Wall -Wextra -pedantic -Wno-unused-parameter -Wnon-virtual-dtor
  -Wno-deprecated-declarations)

cpp_logger_add_c_flags(CMAKE_C_FLAGS
...
if (CPP_LOGGER_WARNINGS_AS_ERRORS)
  cpp_logger_add_cxx_flags(_WERROR_FLAGS -Werror)
  separate_arguments(_WERROR_FLAGS NATIVE_COMMAND "${_WERROR_FLAGS}")
  if (NOT TARGET CPP_LOGGER_CXX_FLAGS_werror)
    add_library(CPP_LOGGER_CXX_FLAGS_werror INTERFACE)
    set_property(TARGET CPP_LOGGER_CXX_FLAGS_werror PROPERTY
      INTERFACE_COMPILE_OPTIONS $<$<COMPILE_LANGUAGE:CXX>:${_WERROR_FLAGS}>)

    add_library(CPP_LOGGER_C_FLAGS_werror INTERFACE)
    set_property(TARGET CPP_LOGGER_C_FLAGS_werror PROPERTY
      INTERFACE_COMPILE_OPTIONS $<$<COMPILE_LANGUAGE:C>:${_WERROR_FLAGS}>)

    # Add the "library" to the export
    install(TARGETS CPP_LOGGER_C_FLAGS_werror EXPORT ${CPP_LOGGER_EXPORTED_TARGETS})
    install(TARGETS CPP_LOGGER_CXX_FLAGS_werror EXPORT ${CPP_LOGGER_EXPORTED_TARGETS})
  endif ()
endif ()
...
if (CMAKE_CXX_COMPILER_ID MATCHES "Clang")
  if (USE_CLANG_LIBCXX)
    cpp_logger_add_cxx_flags(CMAKE_CXX_FLAGS "--stdlib=libc++")
  else (USE_CLANG_LIBCXX)
    if (GCC_TOOLCHAIN)
      cpp_logger_add_cxx_flags(CMAKE_CXX_FLAGS "--gcc-toolchain=${GCC_TOOLCHAIN}")
    endif (GCC_TOOLCHAIN)
  ...
  if (CMAKE_BUILD_TYPE MATCHES Debug)
    ...
    cpp_logger_add_cxx_flags(CMAKE_CXX_FLAGS -fno-omit-frame-pointer)
    cpp_logger_add_c_flags(CMAKE_C_FLAGS -fno-omit-frame-pointer)
  endif ()
endif ()
...
if (CMAKE_CXX_COMPILER_ID MATCHES "Intel")
  ...
  # -openmp_profile
  cpp_logger_add_cxx_flags(CMAKE_CXX_FLAGS -diag-disable=2196 -wd1011 -wd1875 -diag-disable=11074 -diag-disable=11076 ${GCC_INTEROP})
  cpp_logger_add_cxx_flags(CMAKE_C_FLAGS -diag-disable=2196 -wd1011 -wd1875 -diag-disable=11074 -diag-disable=11076 ${GCC_INTEROP})
endif ()
...
set(CMAKE_INSTALL_RPATH "${CMAKE_INSTALL_PREFIX}/${CPP_LOGGER_LIBDIR}")

list(FIND CMAKE_PLATFORM_IMPLICIT_LINK_DIRECTORIES
    "${CMAKE_INSTALL_PREFIX}/${CPP_LOGGER_LIBDIR}" _IS_SYSTEM_DIR)

if (${_IS_SYSTEM_DIR} STREQUAL "-1")
    # Set the install RPATH correctly
    list(APPEND CMAKE_INSTALL_RPATH
      "${CMAKE_INSTALL_PREFIX}/${CPP_LOGGER_LIBDIR}")
endif ()